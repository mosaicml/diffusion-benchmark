...
def compute_ema(model: torch.nn.Module, ema_model: torch.nn.Module, smoothing: float = 0.99):
    ...
    with torch.no_grad():
        ...
        model_params = [p for p in model_params if p.requires_grad]
        ema_model_params = itertools.chain(ema_model.parameters(), ema_model.buffers())
        ema_model_params = [p for p in ema_model_params if p.requires_grad]
...
class EMA(Algorithm):
    ...
    def state_dict(self) -> Dict[str, Any]:
        ...
        for attribute_name in self.serialized_attributes:
            if attribute_name in ['ema_model']:
...
class ShadowModel:
    """A shadow model created from a model, but only containing trainable parameters and buffers."""
    def __init__(self, model: torch.nn.Module):
        self.parameter_list = [copy.deepcopy(p) for p in model.parameters() if p.requires_grad]
        self.buffer_list = [copy.deepcopy(b) for b in model.buffers() if b.requires_grad]
...
def _copy_params(source_model: torch.nn.Module, destination_model: torch.nn.Module):
    ...
    with torch.no_grad():
        ...
        source_params = [s for s in source_params if s.requires_grad]
        destination_params = itertools.chain(destination_model.parameters(), destination_model.buffers())
        destination_params = [d for d in destination_params if d.requires_grad]
...
def _move_params_to_device(model: torch.nn.Module, destination_model: torch.nn.Module):
    ...
    with torch.no_grad():
        ...
        destination_buffers = [d for d in destination_buffers if d.requires_grad]
        buffers = model.buffers()
        buffers = [b for b in buffers if b.requires_grad]
